 ...
 * @retval 0-成功，其他-错误码 
 */
std::string robot_command_thread::MoveJ_ywf(std::string para){
    //JointPos *joint_pos, DescPose *desc_pos, int tool, int user, float vel, float acc, float ovl, ExaxisPos *epos, float blendT, uint8_t offset_flag, DescPose *offset_pos
    int speed,tool,user;
    ...
    if(std::regex_match(head_str,num_match,std::regex("JNT"))){//第一个元素是否满足JNT1这种模式
        //转换参数
        iter_data++;
        double j1 = std::stod(iter_data->str());
        iter_data++;
        double j2 = std::stod(iter_data->str());
        iter_data++;
        double j3 = std::stod(iter_data->str());
        iter_data++;
        double j4 = std::stod(iter_data->str());
        iter_data++;
        double j5 = std::stod(iter_data->str());
        iter_data++;
        double j6 = std::stod(iter_data->str());
        iter_data++;
        speed = std::stoi(iter_data->str());
        iter_data++;
        acc = std::stof(iter_data->str());
        iter_data++;
        blendT = std::stof(iter_data->str());
        //转化为角度结构
        tmp_jnt_pos = JointPos(j1,j2,j3,j4,j5,j6);
        if(_ptr_robot->GetForwardKin(&tmp_jnt_pos,&cartpos) != 0){
            RCLCPP_INFO(rclcpp::get_logger(LOGGER_NAME),"指令错误:MoveJ指令调用正向运动学发生错误");
            return "-1";
        }
    }else if(std::regex_match(head_str,num_match,std::regex("CART"))){
        int index = atol(num_match[2].str().c_str());
        if(index > _cmd_cart_pos_list.size()){
            RCLCPP_INFO(rclcpp::get_logger(LOGGER_NAME),"指令错误:MoveJ输入位置点序号超限");
            return "-1";
