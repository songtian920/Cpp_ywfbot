                   ...
                   rclcpp::NodeOptions().use_intra_process_comms(true))
{
    ...
    if(_socketfd1 == -1){
        ...
        if(res1){
            ...
            _locktimer = this->create_wall_timer(50ms,std::bind(&state_recv_thread::_state_recv_callback,this));//创建一个定时器任务用于获取非实时状态数据，触发间隔为100ms
        }
    }
}
...
void state_recv_thread::_state_recv_callback(){
    ...
    if(recv(_socketfd1,recv_buff,sizeof(recv_buff),0) > -1){
        ...
        // std::cout << msg.j1_cur_pos << std::endl;
        // std::cout << msg.j1_cur_tor << std::endl;
    }
}
