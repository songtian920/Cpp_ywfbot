           ...
           rclcpp::NodeOptions().use_intra_process_comms(true))
{
    ...
    this->declare_parameter<int>("JOG_maxdis",100);//JOG默认单步5mm
    ...
    if(_socketfd1 == -1 || _socketfd2 == -1){
        ...
        if(res1 || res2){
            ...
            {
                RCLCPP_ERROR(rclcpp::get_logger("FrHardwareInterface"),"tcp keepalive lose");
            }
...
int ROS_API::setKeepAlive(int fd, int idle_time,int interval_time,int probe_times){
    int val = 1;
    ...
    if(setsockopt(fd,IPPROTO_TCP,4,&val,sizeof(val)) < 0){
        RCLCPP_ERROR(rclcpp::get_logger("FrHardwareInterface"),"setsockopt TCP_KEEPALIVE: %s",strerror(errno));
        return -1;
    }
    val = interval_time;
    if(val == 0) val = 1;
    if(setsockopt(fd,IPPROTO_TCP,5,&val,sizeof(val)) < 0){
        RCLCPP_ERROR(rclcpp::get_logger("FrHardwareInterface"),"setsockopt TCP_KEEPINTVL: %s",strerror(errno));
        return -1;
    }
    val = probe_times;
    if(setsockopt(fd,IPPROTO_TCP,6,&val,sizeof(val)) < 0){
        RCLCPP_ERROR(rclcpp::get_logger("FrHardwareInterface"),"setsockopt TCP_KEEPCNT: %s",strerror(errno));
        return -1;
    }
}
...
void ROS_API::_selectfunc(std::string func_name){
    ...
    }else if (func_name == "MoveJoint"){
        funcP = &ROS_API::MoveJoint;
    }else if(func_name == "MoveJoint_ywf"){
        funcP = &ROS_API::MoveJoint_ywf;
    }else if (func_name == "MovePose"){
        funcP = &ROS_API::MovePose;
    }else if (func_name == "MovePose_ywf"){
        funcP = &ROS_API::MovePose_ywf;
    }else if (func_name == "MoveL_Pose"){
        funcP = &ROS_API::MoveL_Pose;
    }else if(func_name == "MoveL_Pose_ywf"){
        funcP = &ROS_API::MoveL_Pose_ywf;
    } else if (func_name == "MoveL_Joint"){
        funcP = &ROS_API::MoveL_Joint;
    }else if(func_name == "MoveL_Joint_ywf"){ 
        funcP = &ROS_API::MoveL_Joint_ywf;
    } else if (func_name == "Start"){
        funcP = &ROS_API::Start;
    }else if (func_name == "Pause"){
        funcP = &ROS_API::Pause;
    }else if (func_name == "Resume"){
        funcP = &ROS_API::Resume;
    }
    else
    {
        funcP = NULL;
    ...
    if(!_skip_answer_flag) {//针对servoJT指令，不需要看反馈值直接发送
        ...
        if(recv(_socketfd1,recv_buff,sizeof(recv_buff),0) > -1){
            ...
            if(_ParseRecvData(std::string(recv_buff))){
                ...
                }else if(_recv_data_cmdcount !=  _cmd_counter){
...
//关节空间MoveJoint(para) param=[j1,j2,j3,j4,j5,j6,speed,blendT]
//改动 去除参数tool的指定，joint空间不需要指定工具
int ROS_API::MoveJoint(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendT, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = this->get_parameter("MoveJLC_tool").value_to_string();
  ...
  for (; iter_data != end; iter_data++)
  {
    // std::cout << *iter_data << std::endl;
    count++;
  }
  std::cout << "moveJ() receive params nums:" << count << std::endl;

  if (count == 8)
  {
    JointPos jnt_pos;
    std::cout << "enter moveJoint" << std::endl;
    iter_data = std::regex_token_iterator(para.begin(), para.end(), search_para, -1);
    // std::cout<< "enter movej:"<<atof((*iter_data).str().c_str())<<std::end
    jnt_pos.jPos[0] = atof(iter_data->str().c_str());
    iter_data++;
    // std::cout << "jpos[0]: " <<jnt_pos.jPos[0] << std::endl;
    jnt_pos.jPos[1] = atof(iter_data->str().c_str());
    iter_data++;
    jnt_pos.jPos[2] = atof(iter_data->str().c_str());
    iter_data++;
    jnt_pos.jPos[3] = atof(iter_data->str().c_str());
    iter_data++;
    jnt_pos.jPos[4] = atof(iter_data->str().c_str());
    iter_data++;
    jnt_pos.jPos[5] = atof(iter_data->str().c_str());
    iter_data++;
    //!去除参数tool的指定，joint空间不需要指定工具
    //tool = iter_data->str();
    //iter_data++;
    std::string speed = iter_data->str();
    iter_data++;
    blendT = iter_data->str().c_str();
    std::string para2 = "";
    for (int i = 0; i < 6; i++)
    {
      para2 += std::to_string(jnt_pos.jPos[i]);
      if (i < 5)
      {
        para2 += ",";
      }
    ...
    if (res == -2001)
    {
      ...
      for (int i = 0; i < 6; i++)
      {
        para += std::to_string(jnt_pos.jPos[i]) + ",";
      }
      for (int i = 0; i < 6; i++)
      {
        para += std::to_string(_kin_res[i]) + ",";
      ...
      para = para + tool + "," + user + "," + speed + "," + acc + "," + ovl + "," +
             eaxis1 + "," + eaxis2 + "," + eaxis3 + "," + eaxis4 + "," + blendT + "," +
             offset_flag + "," + offset_pos_x + "," + offset_pos_y + "," + offset_pos_z + "," +
             offset_pos_rx + "," + offset_pos_ry + "," + offset_pos_rz;
      std::string tmp_para = FRAPI_base::command_factry("MoveJ", 1, para);
      // std::cout << "MoveJ发送数据: " << tmp_para << std::endl;
      return _send_data_factory_callback(FRAPI_base::command_factry("MoveJ", ++_cmd_counter, para));
    }
  ...
  {
    std::cout << "指令错误:MoveJ参数输入数量错误" << std::endl;
    return 18;
  }
}


//意微法专用 关节空间MoveJoint(para) param=[j1,j2,j3,j4,j5,j6,speed,blendT]
//改动 去除参数tool的指定，joint空间不需要指定工具
int ROS_API::MoveJoint_ywf(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendT, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = "0";
  ...
  if (count == 9)
  {
    ...
    blendT = iter_data->str().c_str();
    // iter_data++;
    // offset_flag = iter_data->str();
    // iter_data++;
    // offset_pos_x = iter_data->str();
    // iter_data++;
    // offset_pos_y = iter_data->str();
    // iter_data++;
    // offset_pos_z = iter_data->str();
    // iter_data++;
    // offset_pos_rx = iter_data->str();
    // iter_data++;
    // offset_pos_ry = iter_data->str();
    // iter_data++;
    // offset_pos_rz = iter_data->str();
    ...
    {
      std::cout << "指令错误:MoveJoint_ywf指令调用正向运动学发生错误" << std::endl;
      return 16;
    }
  }
  else
  {
    std::cout << "指令错误:MoveJoint_ywf参数输入数量错误" << std::endl;
    return 18;
  }
}


//笛卡尔空间运动 MovePose(param) param=[x,y,z,xRot,yRot,zRot,speed,blendT]
int ROS_API::MovePose(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendT, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = this->get_parameter("MoveJLC_tool").value_to_string();
  ...
  if (count == 8)
  {
    DescPose tmp_cart_pos;
    //std::cout << "enter movePose" << std::endl;
    iter_data = std::regex_token_iterator(para.begin(), para.end(), search_para, -1);
    // std::cout<< "enter movej:"<<atof((*iter_data).str().c_str())<<std::end
    tmp_cart_pos.tran.x = atof(iter_data->str().c_str());
    iter_data++;
    // std::cout << "jpos[0]: " <<jnt_pos.jPos[0] << std::endl;
    tmp_cart_pos.tran.y = atof(iter_data->str().c_str());
    iter_data++;
    tmp_cart_pos.tran.z = atof(iter_data->str().c_str());
    iter_data++;
    tmp_cart_pos.rpy.rx = atof(iter_data->str().c_str());
    iter_data++;
    tmp_cart_pos.rpy.ry = atof(iter_data->str().c_str());
    iter_data++;
    tmp_cart_pos.rpy.rz = atof(iter_data->str().c_str());
    iter_data++;
    //!去除参数tool，由上游计算
    //tool = iter_data->str();
    //iter_data++;
...
//意微法专用 笛卡尔空间运动 MovePose(param) param=[x,y,z,xRot,yRot,zRot,speed,blendT]
int ROS_API::MovePose_ywf(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendT, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = "0";
  ...
  if (count == 9)
  {
    ...
    {
      std::cout << "指令错误:MovePose_ywf指令调用逆向运动学发生错误" << std::endl;
      return 17;
    }
  }
  else
  {
    std::cout << "指令错误:MovePose_ywf参数输入数量错误" << std::endl;
    return 18;
  }
}
...
//笛卡尔空间运动 MoveL_Joint(param) param=[j1,j2,j3,j4,j5,j6,speed,blendT]
int ROS_API::MoveL_Joint(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendR, search, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = this->get_parameter("MoveJLC_tool").value_to_string();
  ...
  if (count == 8)
  {
    ...
    if (res == -2001)
    {
      ...
      std::string tmp_para = FRAPI_base::command_factry("MoveL", 1, para);
      para = para + tool + "," + user + "," + speed + "," + acc + "," +
             ovl + "," + blendR + "," + eaxis1 + "," + eaxis2 + "," +
             eaxis3 + "," + eaxis4 + "," + search + "," + offset_flag + "," +
    ...
    {
      std::cout << "指令错误:MoveL_Joint指令调用正向运动学发生错误" << std::endl;
      return 16;
    }
  }
  else
  {
    std::cout << "指令错误:MoveL_Joint参数输入数量错误" << std::endl;
    return 18;
  }
}

//意微法专用笛卡尔空间运动 MoveL_Joint(param) param=[j1,j2,j3,j4,j5,j6,speed,blendT]
int ROS_API::MoveL_Joint_ywf(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendR, search, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = "0";
  ...
  if (count == 9)
  {
    ...
    {
      std::cout << "指令错误:MoveL_Joint_ywf指令调用正向运动学发生错误" << std::endl;
      return 16;
    }
  }
  else
  {
    std::cout << "指令错误:MoveL_Joint_ywf参数输入数量错误" << std::endl;
    return 18;
  }
}

//笛卡尔空间运动 MoveL_Pose(param) param=[x,y,z,xRot,yRot,zRot,speed,blendT]
int ROS_API::MoveL_Pose(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendR, search, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = this->get_parameter("MoveJLC_tool").value_to_string();
  ...
  if (count == 8)
  {
    ...
    if (res == -2001)
    {
      ...
      para = para + std::to_string(tmp_cart_pos.rpy.rz) + ",";
      para = para + tool + "," + user + "," + speed + "," + acc + "," + ovl + "," +
             blendR + "," + eaxis1 + "," + eaxis2 + "," + eaxis3 + "," + eaxis4 + "," +
             search + "," + offset_flag + "," + offset_pos_x + "," + offset_pos_y + "," +
             offset_pos_z + "," + offset_pos_rx + "," + offset_pos_ry + "," + offset_pos_rz;
      return _send_data_factory_callback(FRAPI_base::command_factry("MoveL", ++_cmd_counter, para));
    }
    else
    {
...
//笛卡尔空间运动 MoveL_Pose(param) param=[x,y,z,xRot,yRot,zRot,speed,blendT]
int ROS_API::MoveL_Pose_ywf(std::string para)
{
  std::string tool, user, acc, ovl, eaxis1, eaxis2, eaxis3, eaxis4, blendR, search, offset_flag, offset_pos_x,
      offset_pos_y, offset_pos_z, offset_pos_rx, offset_pos_ry, offset_pos_rz;
  tool = "0";
  ...
  if (count == 9)
  {
    ...
    if (res == -2001)
    {
      ...
      para = para + tool + "," + user + "," + speed + "," + acc + "," + ovl + "," +
             ...
             search + "," + offset_flag + "," + offset_pos_x + "," + offset_pos_y + "," +
             offset_pos_z + "," + offset_pos_rx + "," + offset_pos_ry + "," + offset_pos_rz;
      int res_cmd = _send_data_factory_callback(FRAPI_base::command_factry("MoveL", ++_cmd_counter, para));
      return res_cmd;
    }
    else
    {
      std::cout << "指令错误:MoveL_Pose_ywf指令调用逆向运动学发生错误" << std::endl;
      return 17;
    }
  }
  else
  {
    std::cout << "指令错误:MoveL_Pose_ywf参数输入数量错误" << std::endl;
    return 18;
  }
}
...
int ROS_API::NewSplinePoint(std::string para){
    ...
    if(std::regex_match(head_str,num_match,std::regex("(CART)([0-9]*)"))){
        ...
        para2 = para2 + std::to_string(tmp_cart_pos.rpy.rz) + ",";
        para2 += "-1";
        int res = _send_data_factory_callback(FRAPI_base::command_factry("GetInvearseKin",++_cmd_counter,para2));//求解逆向运动学
        if(res == -2001){
            para.clear();
            for(int i=0;i<6;i++){
                para += std::to_string(_kin_res[i]) + ","; 
...
int ROS_API::Start(std::string para)
{
  // empty para
  para.clear();
  return _send_data_factory_callback(FRAPI_base::command_factry("Start", ++_cmd_counter, para));
}

int ROS_API::Pause(std::string para)
{
  // empty para
  para.clear();
  return _send_data_factory_callback(FRAPI_base::command_factry("Pause", ++_cmd_counter, para));
}

int ROS_API::Resume(std::string para)
{
  // empty para
  para.clear();
  return _send_data_factory_callback(FRAPI_base::command_factry("Resume", ++_cmd_counter, para));
}
